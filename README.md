...
`event` はサイト上では **EVENT PLANNING** と表示される（`lib/works.ts` の
`categoryLabel`）。「EVENT」だけだと、イベントを撮った映像との区別がつかないため。
...
成果物がイベントそのもので、映像やSNSと並ぶ「作ったもの」だから。

`イベント` は tags にも残す。両者は別物で、使い分けはこう。

| 案件 | category | tags |
|---|---|---|
| イベントを企画・運営した | `event` | — |
| イベントを撮影した映像 | `movie` | `イベント` |
...
| 5 | 実績の動画・詳細本文 | 詳細ページは OVERVIEW / STILLS が無い場合その枠を出さない |

### 詳細ページの映像

`videoUrl` の配信元で表示が変わる。写真だけの案件でも文言は出さない
（「準備中」のような表示は、動画が存在しない案件には誤解を招くため）。

| videoUrl | 表示 |
|---|---|
| Vimeo / YouTube | 開いた時点で1回だけ自動再生（無音・操作パネルあり） |
| Instagram | 静止画 + PLAY（埋め込みで再生できないため外部リンク） |
| なし | 静止画のみ |

自動再生はブラウザの規定で無音でないと止まるので muted で始める。
音は操作パネルから出せる。
